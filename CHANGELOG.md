...
- `OtpConfirmationStep` (`@cellarnode/auth/react`) — a shared six-digit OTP confirmation interaction (auto-advance, backspace/arrow nav, paste, `autocomplete="one-time-code"`, a one-flight request-on-mount guard, a resend countdown bound to the server's real `resendAvailableAt`, invalid/expired/max-attempts/rate-limit error mapping, reduced-motion and mobile-sized rendering) built on the existing `InputOTP` primitives. Transport- and copy-agnostic: callers supply `onRequestCode`/`onVerifyCode` and an explicit `labels` prop (CEL-2087).
